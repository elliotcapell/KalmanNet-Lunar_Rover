...
# from LunarRover import train_valid_indices
...
class Pipeline_EKF:
    ...
    def NNTrain(self, SysModel, cv_input, cv_target, train_input, train_target, valid_indices, path_results, \
        ...
        for ti in range(0, self.N_steps):
            ...
            train_target_batch = torch.zeros([self.N_B, 6, SysModel.T]).to(self.device)
            x_out_training_batch = torch.zeros([self.N_B, SysModel.n, SysModel.T]).to(self.device)
            train_batch_indexes = torch.zeros(self.N_B).to(self.device)
            ...
            for index in n_e:
                if self.args.randomLength:
                    ...
                    train_batch_indexes[ii] = valid_indices[index]
                ii += 1
            ...
            if(randomInit):
                ...
                for index in n_e:
                    ...
                    ii += 1
                self.model.InitSequence(train_init_batch, SysModel.T, train_batch_indexes, 0)
            else:
                self.model.InitSequence(\
                SysModel.m1x_0.reshape(1,SysModel.m,1).repeat(self.N_B,1,1), SysModel.T, train_batch_indexes, 0)
            
            # Forward Computation
            for t in range(0, SysModel.T):
                x_out_training_batch[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(y_training_batch[:, :, t],2), train_batch_indexes, t))
            ...
            if (self.args.CompositionLoss):
                ...
                for t in range(SysModel.T):
                    y_hat[:,:,t] = torch.squeeze(SysModel.h(torch.unsqueeze(x_out_training_batch[:,:,t],2), train_batch_indexes, t))
                ...
                if(MaskOnState):### FIXME: composition loss, y_hat may have different mask with x
                    ...
                    if self.args.randomLength:
                        ...
                        MSE_trainbatch_linear_LOSS = self.alpha * self.loss_fn(x_out_training_batch[:, :6, :], train_target_batch) + (1-self.alpha) * self.loss_fn(y_hat, y_training_batch)
                ...
                if(MaskOnState):
                    ...
                    if self.args.randomLength:
                        ...
                        MSE_trainbatch_linear_LOSS = self.loss_fn(x_out_training_batch[:, :6, :], train_target_batch)
            ...
            with torch.no_grad():
                ...
                cv_batch_indexes = valid_indices[torch.arange(self.N_E, self.N_E + self.N_CV)]

                # Init Sequence
                if(randomInit):
                    if(cv_init==None):
                        self.model.InitSequence(\
                        SysModel.m1x_0.reshape(1,SysModel.m,1).repeat(self.N_CV,1,1), SysModel.T_test, cv_batch_indexes, 0)
                    else:
                        self.model.InitSequence(cv_init, SysModel.T_test, cv_batch_indexes, 0)                       
                else:
                    self.model.InitSequence(\
                        SysModel.m1x_0.reshape(1,SysModel.m,1).repeat(self.N_CV,1,1), SysModel.T_test, cv_batch_indexes, 0)

                for t in range(0, SysModel.T_test):
                    x_out_cv_batch[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(cv_input[:, :, t],2), cv_batch_indexes, t))
                ...
                if(MaskOnState):
                    ...
                    if self.args.randomLength:
                        ...
                        MSE_cvbatch_linear_LOSS = self.loss_fn(x_out_cv_batch[:, :6, :], cv_target)
    ...
    def NNTest(self, SysModel, test_input, test_target, valid_indices, path_results, MaskOnState=False,\
     randomInit=False,test_init=None,load_model=False,load_model_path=None,\
        ...
        test_batch_indexes = valid_indices[torch.arange(self.N_E + self.N_CV, self.N_E + self.N_CV + self.N_T)]
        ...
        if (randomInit):
            self.model.InitSequence(test_init, SysModel.T_test, test_batch_indexes, 0)               
        else:
            self.model.InitSequence(SysModel.m1x_0.reshape(1,SysModel.m,1).repeat(self.N_T,1,1), SysModel.T_test, test_batch_indexes, 0)         
        
        for t in range(0, SysModel.T_test):
            x_out_test[:,:, t] = torch.squeeze(self.model(torch.unsqueeze(test_input[:,:, t],2), test_batch_indexes, t))
        ...
        for j in range(self.N_T):# cannot use batch due to different length and std computation  
            if(MaskOnState):
                ...
                if self.args.randomLength:
                    ...
                    self.MSE_test_linear_arr[j] = loss_fn(x_out_test[j,:6,:], test_target[j,:,:]).item()
